// inm showcase v2 — Tokens: semantic token table with live swatches + light/dark values
function Tokens() {
  const tokens = [
      ...
      <div className="wrap">
        ...
        <div className="token-table" role="table" aria-label="Semantic tokens">
          <div className="tk-row head" role="row">
            <span role="columnheader" aria-label="Swatch"></span>
          ...
          {tokens.map((t) => (
            <div className="tk-row" role="row" key={t.token}>
              <span className="tk-swatch" style={{ background: "var(--color-" + t.token + ")" }} aria-hidden="true"></span>
