...
展示站還內建了一個可互動的 console 儀表板 demo，篩選列、sidebar、指標卡全都有。  
homo 特有的一次全端上來。
...
不用 Tailwind 也可以直接拿 [colors_and_type.css](./colors_and_type.css) / [styles.css](./styles.css) 當 token，link 進去就能用了。  
這事可以的（確信
...
- 不要把原始 `#A1736B` 當小字放在 `#C9BFB6` 上面，對比不足不可避。
- `#979F9B` 是支援色，不是正文色。誤用成主色調，迷霧閱讀不可避（惱
